...
export function activate(context: vscode.ExtensionContext) {
	...
	const onResponsiveView = vscode.commands.registerCommand(
		"prismaStudio.responsiveView",
		() => {
			if (PreviewPanel.currentPanel) {
	...
	const onScreenView = vscode.commands.registerCommand(
		"prismaStudio.screenView",
		() => {
			if (PreviewPanel.currentPanel) {
	...
	const onOpenDevTools = vscode.commands.registerCommand(
		"prismaStudio.openDevTools",
		() => {
			if (PreviewPanel.currentPanel) {
	...
	const onOpenInBrowser = vscode.commands.registerCommand(
		"prismaStudio.openInBrowser",
		() => {
			if (PreviewPanel.currentPanel) {
	...
	const onStartPrisma = vscode.commands.registerCommand(
		"prismaStudio.startPrisma",
		async () => {
			await PrismaManager.startPrismaStudio();
		},
	);
	const onStopPrisma = vscode.commands.registerCommand(
		"prismaStudio.stopPrisma",
		async () => {
			await PrismaManager.stopPrismaStudio();
		},
	);
	const onConfig = vscode.commands.registerCommand(
		"prismaStudio.config",
		async () => {
...
class PreviewPanel {
	...
	public static readonly viewType = "prismaStudio";
	...
	public static createOrShow(extensionUri: vscode.Uri) {
		...
		if (PreviewPanel.currentPanel) {
			PreviewPanel.currentPanel._panel.reveal(
				PreviewPanel.currentPanel._panel.viewColumn,
			);
			return;
		}
...
class PrismaManager {
		...
		try {
			...
			if (isPortInUse) {
				const action = await vscode.window.showWarningMessage(
					`Port ${port} is already in use. Stop existing process?`,
					"Stop and Start", "Cancel"
				);
			...
			PrismaManager.process.stdout?.on("data", (data: any) => {
			...
			PrismaManager.process.stderr?.on("data", (data: any) => {
			...
			PrismaManager.process.on("close", (code: number) => {
	...
	private static async getSchemaPath(): Promise<string | undefined> {
